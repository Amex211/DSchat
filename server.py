...
class ChatServer:
    ...
    def start(self):
        ...
        time.sleep(2)
        self.start_election()
        print(f"Server {self.server_id} started on {self.ip}:{self.port}")
    ...
    def _send_heartbeat(self):
        print("[Logger] I'm now sending heartbeats to all known servers")
        msg = {
    ...
    def _heartbeat_monitor(self):
        while self.running:
            print(f"[Monitor] State: {self.state}, Leader: {self.leader_id}, Running: {self.running}")
            if self.state == ServerState.FOLLOWER and self.leader_id:
                print(f"[Monitor] Check activated. State: {self.state}, Leader ID: {self.leader_id}")
                last = self.last_heartbeat.get(self.leader_id, 0)
                delta = int(time.time() - last)
                print(f"[Monitor] Checking leader {self.leader_id}, last signal {delta}s ago")
                if delta > 6:
                    print("[Logger] Leader not reachable – starting new election")
                    self.start_election()
            ...
            if self.state == ServerState.LEADER:
                ...
                for sid, last in self.last_heartbeat.items():
                    if sid != self.server_id and now - last > 8:
                        print(f"[Monitor] Removing server {sid} (no heartbeat for {int(now - last)}s)")
                        failed.append(sid)
                for sid in failed:
                    print(f"[Logger] Server {sid} removed – no connection anymore")
    ...
    def _route_udp_message(self, msg, addr):
        ...
        if t == MessageType.SERVER_ANNOUNCE.value:
            ...
            if sid != self.server_id and sid not in self.servers:
                ...
                print(f"[Logger] New server discovered: ID={sid}, IP={ip}, PORT={port}")
                if sid > self.server_id:
                    print(f"[Logger] Discovered server {sid} has higher ID than me ({self.server_id}) – starting election")
                    self.state = ServerState.FOLLOWER
                    self.leader_id = None
                    print("[Logger] Switching to FOLLOWER – waiting for new leader")
                    threading.Thread(target=self.start_election, daemon=True).start()
        elif t == MessageType.HEARTBEAT.value:
            leader_id = msg.get("leader_id")
            sender_id = msg.get("sender_id")
            print(f"[Logger] HEARTBEAT received from leader {leader_id}")
            ...
            if leader_id:
                ...
                if leader_id != self.server_id:
                    ...
                    self.state = ServerState.FOLLOWER
                    print(f"[Logger] Setting leader ID to {leader_id}, switching to FOLLOWER")
            ...
            print(f"[Logger] Election request from server {msg.get('sender_id')}")
            if msg.get("sender_id") < self.server_id:
                print(f"[Logger] Sending OK to server {msg.get('sender_id')}")
            ...
            print(f"[Logger] OK received from server {msg.get('sender_id')}")
            self.ok_received = True
            self.election_in_progress = False
        elif t == MessageType.COORDINATOR.value:
            new_leader = msg.get("leader_id")
            print(f"[Logger] COORDINATOR received from server {new_leader}")
            if new_leader != self.server_id:
                self.leader_id = new_leader
                self.state = ServerState.FOLLOWER
                print(f"[Logger] Switching to FOLLOWER – leader is now {new_leader}")
            else:
                print(f"[Logger] Ignoring COORDINATOR from myself ({new_leader})")
            self.election_in_progress = False

    def start_election(self):
        print(f"[Logger] Starting election from server {self.server_id}")
    ...
    def become_leader(self):
        ...
        self.election_in_progress = False
        print(f"[Logger] Server {self.server_id} is now leader")
